...
class Config(object):
    ...
    def __init__(self, genome_type, reproduction_type, species_set_type, stagnation_type, filename):
        ...
        # error detection if config file not present
        ...
        #param_list_name -list with names of NEAT section parameters as strings
        param_list_names = []
        ...
        #param_dict - dict with user input name(key) and value(value) as in config file
        param_dict = dict(parameters.items('NEAT'))
        # print(param_dict)


        #this is for debugging if unknown parameter is input by the user
        unknown_list = [x for x in iterkeys(param_dict) if not x in param_list_names]
        # print(unknown_list)
        if unknown_list:
            if len(unknown_list) > 1:
        ...
        #genome_dict - dict with user input parameter(key) and value(value) as in config file under genome section
        genome_dict = dict(parameters.items(genome_type.__name__))
        # print(genome_dict)
        #genome_config - object whose attributes names are parameters input by the user and value the corresponding values.
        self.genome_config = genome_type.parse_config(genome_dict)
        

        #species_dict - dict with user input parameter(key) and value(value) as in config file under species section
        ...
        #stagnation_dict - dict with user input parameter(key) and value(value) as in config file under stagnation section
        ...
        #reproduction_dict - dict with user input parameter(key) and value(value) as in config file under reproduction section
